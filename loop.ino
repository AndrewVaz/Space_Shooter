...
void loop() {
  ...
  if (start > 0)
  {
    ...
    // This statement OLEDs the rules on the OLED screen
      ...
      // When nothing is pressed OLED displays the start of the game
      if (btnp <= 0) {
        Oled.setCursor(3, 3);
      ...
      if (btnp == 1) {
        ...
        // When the button is pressed and you haven't reached the game limit(10 tries)
        // do this statement
        if (button_state == true && tries <= 9) {
          shoot();
          tries++;

          // This statement determines the range of the player POT
          // If the player's POT value is greater than/less than the random value do this
          // The NOT statement stops the code from going into the greater than/less than
          // statements so the code knows that the player has reached the target/goal
          if ((value > randnum) && !(value + 2 == randnum || value - 2 == randnum)) {
            sum = value - randnum; // this calculates the distance of the POT to the random value
            // Depending on what the sum is, a fixed amount of lights turn on
            // The greater the sum the fewer the lights
            if (sum >= 0 && sum <= 5)
            {
              on4(value, randnum); // This takes the two values to determine which direction the lights turn on 
            }
